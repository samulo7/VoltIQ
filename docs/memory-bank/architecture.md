...
版本：V1.33  
状态：一期基线已锁定（Step 1-18 与 Step 21 已完成并通过用户验证；Step 19-20 已完成工程实现待用户验证；Step 12 测速能力含 TTFT 统计）  
更新日期：2026-03-23
...
## 32. Step 21 基础数据看板页面基线（已验收）
- 落地目录：
  - `frontend/src/services/voltiq/metrics.ts`。
  - `frontend/src/pages/metrics/index.tsx`。
- 页面能力基线：
  - 已替换 StepPlaceholder，占位页升级为可用页面，支持“筛选区 + 指标卡 + 按日明细表”结构。
  - 已接入 `GET /api/v1/metrics/overview`，展示 `lead_count`、`effective_lead_count`、`deal_count`、`conversion_rate`。
  - 已接入日期范围筛选，按 `YYYY-MM-DD` 传递 `start_date/end_date`，并支持重置为“今日”。
  - 已落地请求状态体验：加载、错误、空数据与正常态，并提供重试/刷新入口。
  - 已落地展示口径：仅做前端格式化（如转化率百分比显示），不重算后端核心指标。
- 权限渲染基线：
  - sales：可查看本人作用域看板数据（后端作用域约束继续生效）。
  - manager：可查看全量看板数据。
  - operator：保持无数据看板菜单入口（由 Step 16 权限渲染基线控制）。
- 测试基线：
  - 已执行前端类型检查：`pnpm -C frontend run tsc`（2026-03-23）。
- 验收状态：
