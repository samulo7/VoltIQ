...
版本：V1.31  
状态：Step 1-18 与 Step 21 已完成并通过用户验证；Step 19-20 已完成工程实现待用户验证  
更新日期：2026-03-23
...
执行门禁（2026-03-23）：Step 18（CRM 跟进与商机页面）与 Step 21（基础数据看板）已通过用户验证；Step 19 与 Step 20 已按用户指令启动并完成工程实现，待用户验证；Step 22 门禁已解除，是否启动以用户指令为准。前端产品化改造计划（`docs/memory-bank/frontend_productization_execution_plan_v1.md`）仅在 V1 阶段完成并验收后启动。
...
- 执行边界：
  ...
  - Step 17（线索管理页面）已按用户明确指令完成并通过用户测试验证（2026-03-21）。

### 17. 线索管理页面（已完成，并通过用户验证）
- 指令：实现线索列表、筛选、编辑、分配与去重结果展示。
- 验证：页面操作可触发接口并正确展示结果。
- 当前产出：
  - 已新增 `frontend/src/services/voltiq/leads.ts`，封装 `list/create/update/assign/merge/detail` 接口请求与类型。
  - 已改造 `frontend/src/pages/leads/index.tsx`，落地线索管理完整页面（列表、分页、筛选、编辑、新增、分配、手动合并）。
  - 已落地创建去重结果展示：`POST /api/v1/leads` 返回 `action=merged` 时在页面显式提示 `merge_reason`。
  - 已落地分配负责人交互口径：`operator` 通过 UUID 手动输入完成 `owner_user_id` 分配。
  - 已落地角色渲染：`operator` 可新增/编辑/分配/手动合并，`sales` 可新增/编辑，`manager` 只读。
  - 已完成联调修复：前端默认 API 地址统一 `9000`，并补齐 `X-Actor-Role`、`X-Actor-User-Id` 请求头以兼容当前后端线索模块鉴权依赖。
  - 已增强手动合并交互反馈：页面顶部常驻展示“最近一次手动合并结果”。
- 本地验证：
  - 已执行 `pnpm -C frontend run tsc`，结果通过（`tsc --noEmit` 成功，2026-03-21）。
- 执行边界：
  - Step 17 已按用户指令完成并通过用户测试验证（2026-03-21）。
  - Step 18 门禁已解除，可按用户指令启动。

### 18. CRM 跟进与商机页面（已完成，并通过用户验证）
- 指令：实现跟进记录列表与商机阶段流转 UI。
- 验证：新增跟进实时刷新，阶段流转准确。
- 当前产出：
  - 已新增 `frontend/src/services/voltiq/crm.ts`，封装 `follow-ups`、`opportunities`、`deals` 的请求与类型。
  - 已改造 `frontend/src/pages/crm/follow-ups/index.tsx`，实现跟进列表分页、筛选（线索/客户/创建人/时间）、新增、编辑、删除。
  - 已落地跟进新增“下拉选择线索”交互（可搜索），并保留可选 `customer_id` 录入。
  - 已改造 `frontend/src/pages/crm/opportunities/index.tsx`，实现商机列表分页、筛选、阶段统计、阶段流转（含 `lost_reason` 校验）与商机内联创建成单。
  - 已改造 `frontend/src/pages/crm/deals/index.tsx`，实现成单列表分页、按商机/日期筛选、基于谈判中商机下拉创建成单。
  - 已落地权限渲染：`sales` 可写，`manager` 只读；`operator` 继续无 CRM 菜单入口。
- 本地验证：
  - 已执行 `pnpm -C frontend run tsc`，结果通过（`tsc --noEmit` 成功，2026-03-21）。
- 执行边界：
  - Step 18 已按用户指令完成并通过用户测试验证（2026-03-21）。
  - Step 19 门禁已解除，是否启动以用户指令为准。

### 19. 内容生成页面（已完成工程实现，待用户验证）
- 指令：实现三类内容任务提交与结果展示。
- 验证：提交后可看到任务状态变化与结果区域更新。
- 当前产出：
  - 已新增 frontend/src/services/voltiq/content.ts，封装 create/list/detail 接口与内容任务类型。
  - 已替换 frontend/src/pages/content/tasks/index.tsx 占位页，落地任务列表分页、筛选（task_type/status/created_by/created_at）、新建任务弹窗与详情结果区。
  - 已按确认口径实现前端过渡态：提交时显示“任务提交中”，提交成功后刷新列表并显示最新任务状态。
  - 已落地权限渲染：operator 可创建+查看，manager 只读查看，sales 保持无菜单入口。
- 本地验证：
  - 已执行 pnpm -C frontend run tsc（2026-03-21）。
- 执行边界：
  - Step 19 已按用户指令完成工程实现，待用户测试确认。
  - Step 20 已按用户指令启动并完成工程实现，待用户测试验证。

### 20. 智能客服页面（已完成工程实现，待用户验证）
- 指令：实现问答对话框并展示回答依据。
- 验证：连续提问可得到上下文相关回答。
- 当前产出：
  - 已新增 `frontend/src/services/voltiq/kb.ts`，封装 `listSessions` 与 `chat` 接口请求及响应类型。
  - 已替换 `frontend/src/pages/kb/sessions/index.tsx` 占位页，落地“会话列表 + 问答对话”双栏页面。
  - 已接入会话分页查询 `GET /api/v1/kb/sessions`，支持会话刷新、分页与当前会话切换。
  - 已接入问答接口 `POST /api/v1/kb/sessions/chat`，支持首次提问自动建会话、后续带 `session_key` 连续提问。
  - 已落地回答依据展示：assistant 消息显示 `sources`（知识库/文档/评分/片段内容）。
  - 已落地发送过渡态与新会话入口：发送中禁用输入并显示 loading；“新会话”首次发送后自动获得会话键。
  - 已落地权限渲染：`operator` 与 `manager` 可用，`sales` 保持无菜单入口。
- 本地验证：
  - 已执行 `pnpm -C frontend run tsc`（2026-03-21）。
- 执行边界：
  - Step 20 已按用户指令完成工程实现，待用户测试确认。
  - Step 21 已按用户指令启动并完成工程实现，且已通过用户测试验证（2026-03-23）。

### 21. 基础数据看板（已完成并通过用户验证）
- 指令：实现基础指标看板（线索、转化、成单）。
- 验证：看板数据与后端接口一致。
- 当前产出：
  - 已新增 `frontend/src/services/voltiq/metrics.ts`，统一封装 `GET /api/v1/metrics/overview` 请求与响应类型。
  - 已替换 `frontend/src/pages/metrics/index.tsx` 占位页，落地“筛选区 + 指标卡 + 按日明细表”看板页面。
  - 已接入日期范围筛选，前端按 `YYYY-MM-DD` 传递 `start_date/end_date`，与后端口径保持一致。
  - 已接入状态体验：加载、错误、空数据、正常展示，并提供“重试/刷新”入口。
  - 已落地权限渲染：`sales` 与 `manager` 可用，`operator` 保持无数据看板菜单入口。
- 本地验证：
  - 已执行 `pnpm -C frontend run tsc`（2026-03-23）。
- 验收状态：
  - Step 21 已通过用户测试验证（确认日期：2026-03-23）。
- 执行边界：
  - Step 22 门禁已解除，是否启动 Step 22 以用户指令为准。
